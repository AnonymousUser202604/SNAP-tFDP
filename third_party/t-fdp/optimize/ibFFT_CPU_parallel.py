...
import time
import numba
from .ibFFT_CPU_NumbaKernel_parallel import *
...
def ibFFT_repulsive_parallel(Y, n_interpolation_points, intervals_per_integer, min_num_intervals, gamma, paraFactor, n_threads):
    max_coord = Y.max()
    min_coord = Y.min()
    N = Y.shape[0]
    ...
    fft_kernel_tilde = rfft2(circulant_kernel_tilde, threads=n_threads)
    ...
    fft_w = rfft2(mat_w, threads=n_threads)
    rmut = fft_w * fft_kernel_tilde
    output = irfft2(rmut, threads=n_threads)
...
def ibFFT_CPU_parallel(pos, edgesrc, edgetgt, n_interpolation_points=3, intervals_per_integer=1.0, min_num_intervals=100,
                       alpha=0.1, beta=8, gamma=2, max_iter=300, combine=True, seed=None, n_threads=8):
    ...
    for it in range(max_iter):
        ...
        dC += d3alpha * ibFFT_repulsive_parallel(pos, n_interpolation_points,
                                                 intervals_per_integer, min_num_intervals, np.float32(gamma), np.float32(paraFactor), n_threads)
        ApplyForce(dC, pos, N)
