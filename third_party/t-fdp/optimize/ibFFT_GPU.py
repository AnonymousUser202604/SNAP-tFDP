...
def ibFFT_repulsive(Y, n_interpolation_points, intervals_per_integer, min_num_intervals, gamma, paraFactor):
    ...
    Circulant_kernel_tilde_cu(grid_dim, block_dim,
...
def ibFFT_GPU(pos, edgesrc, edgetgt, n_interpolation_points=3, intervals_per_integer=1.0, min_num_intervals=100,
    ...
    for it in range(max_iter):
        ...
        dC += d3alpha * ibFFT_repulsive(pos, n_interpolation_points,
                                        intervals_per_integer, min_num_intervals, cnp.float32(gamma),
        ...
        if ((it + 1) % 5 == 0):
            print(".", end="")
            sys.stdout.flush()
        free, total = cnp.cuda.Device().mem_info
        used_MB = (total - free) / 1024 ** 2
        print(f"iter {it + 1}, GPU memory used: {used_MB:.2f} MB")
    print("\n", end="")
    ed = time.time()
