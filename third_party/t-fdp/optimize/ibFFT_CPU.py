...
def ibFFT_repulsive_wyf(Y, n_interpolation_points, intervals_per_integer, min_num_intervals, gamma, paraFactor):
    max_coord = Y.max()
    min_coord = Y.min()
    N = Y.shape[0]
    n_boxes_per_dim = int(np.minimum(np.sqrt(16*N), np.maximum(np.sqrt(4 * N/np.log(N)), np.maximum(
    ...
    # half_kernel = 31.25 * paraFactor * np.exp(-(np.arange(n_interpolation_points_1d)**2 + (
    #         np.arange(n_interpolation_points_1d)**2).reshape(-1, 1)) * whsquare / 0.64)

    # half_kernel = 3.125 * paraFactor * np.exp(-(np.arange(n_interpolation_points_1d)**2 + (
    #         np.arange(n_interpolation_points_1d)**2).reshape(-1, 1)) * whsquare / 0.64)

    radius = 0.4
    magnitude = 1

    half_kernel = paraFactor * magnitude / (2 * radius * radius)  * np.exp(-(np.arange(n_interpolation_points_1d)**2 + (
        np.arange(n_interpolation_points_1d)**2).reshape(-1, 1)) * whsquare / (4 * radius * radius))
...
def ibFFT_CPU_wyf(pos, edgesrc, edgetgt, n_interpolation_points=3, intervals_per_integer=1.0, min_num_intervals=100,
              alpha=0.1, beta=8, gamma=2, max_iter=300, combine=True, seed=None):
    paraFactor = 1.0
    if (alpha != 0):
        paraFactor /= alpha
    ...
    for it in range(max_iter):
        if combine:
            ...
            if it == (19*max_iter//20):
                n_interpolation_points = 3
        AttrForce_wyf(attr_force, dC, pos, edgesrc, edgetgt, bias, N, np.float32(
            beta), d3alpha)
        dC += attr_force
        dC -= 0.01 * d3alpha * \
            torch.normal(0, 1, size=pos.shape).numpy().astype(np.float32)
        dC += d3alpha * ibFFT_repulsive_wyf(pos, n_interpolation_points,
                                        intervals_per_integer, min_num_intervals, np.float32(gamma), np.float32(paraFactor))
        ApplyForce(dC, pos, N)
        # 1 - pow(0.02, 1 / 300) = 0.012955423246736264
        d3alpha += (d3alphaMin-d3alpha)*0.012955423246736264
