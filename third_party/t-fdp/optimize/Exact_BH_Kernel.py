...
#         candidates = np.array([j for j in range(N) if j != i], dtype=np.int64)

#         K = 3
#         neighbors = np.random.choice(candidates, K, replace=False)
...
#             # R = alpha * np.power(dsqare + 1e-12, -1)
...
@njit(parallel=False, cache=True)
def ReplForce_wyf(repl_force, pos, N, a, c, alpha, degrees):
    neg_sample = 5
    for i in prange(N):
        repl_force[i][0] = 0
        repl_force[i][1] = 0
        for j in range(N):
            ...
            mv1 = pos[i][1] - pos[j][1]
            dsqare = mv0 * mv0 + mv1 * mv1
            R = alpha * a * np.power(1.0 + dsqare, -c) * (degrees[i] + degrees[j]) * neg_sample / N / 2
            repl_force[i][0] += R * mv0
            repl_force[i][1] += R * mv1
