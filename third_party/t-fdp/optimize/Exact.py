...
def Exact_wyf(pos, edgesrc, edgetgt, alpha=0.1, beta=8, gamma=2, max_iter=300, seed=None, filename=None):
    angle = 0.6
    neg_c = 1.0
    verbose = 9
    n_components = 2
    ...
    degrees = edgesrc[1:] - edgesrc[:-1]

    # betweenness = np.loadtxt(f"/home/sdu/wyf/t-fdp-new/betweenness_compute/{filename}.txt", usecols=1)

    # /home/sdu/wyf/t-fdp-new/betweenness_compute
    ...
    for it in range(max_iter):
        ...
        dC -= 0.01 * d3alpha * \
            torch.normal(0, 1, size=pos.shape).numpy().astype(np.float32)
        ReplForce_wyf(repl_force, pos, N, paraFactor, gamma, d3alpha, degrees)
        dC += repl_force
        ApplyForce(dC, pos, N)
        # 1 - pow(0.02, 1 / 300) = 0.012955423246736264
        d3alpha += (d3alphaMin-d3alpha)*0.012955423246736264
