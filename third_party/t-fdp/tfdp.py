...
from optimize.ibFFT_CPU import ibFFT_CPU, ibFFT_CPU_wyf
from optimize.ibFFT_CPU_parallel import ibFFT_CPU_parallel
from optimize.ibFFT_GPU import ibFFT_GPU

from optimize.Exact import Exact, Exact_wyf
...
def readgraph(filename, returnDijMat=False):
    ...
    edges = dask.dataframe.read_csv(filename, header=None,
                                    names=["src", "tgt"], sep=' ', comment="%", dtype={"src": np.int32, "tgt": np.int32})
...
class tFDP:
    def __init__(self, init='pmds', algo='ibFFT_CPU', max_iter=300, alpha=0.1, beta=8.0, gamma=2.0,
                 n_interpolation_points=3, intervals_per_integer=1.0, min_num_intervals=100, combine=True, randseed=None, filename=None, n_threads=8):
    ...
    def optimization(self, graph=None, edgesrc=[], edgetgt=[], init=None, algo=None):
        ...
        if type(self.init) is np.ndarray and self.init.shape == (graph.shape[0], 2):
            init = self.init
        elif self.init == 'pmds':
            if (self.randseed):
                np.random.seed(self.randseed)
        ...
        if (self.algo == 'ibFFT_CPU_parallel'):
            posres, t = ibFFT_CPU_parallel(init, edgesrc, edgetgt, n_interpolation_points, self.intervals_per_integer, self.min_num_intervals,
                                           self.alpha, self.beta, self.gamma, self.max_iter, self.combine, self.randseed, self.n_threads)

        if (self.algo == 'ibFFT_CPU_wyf'):
            posres, t = ibFFT_CPU_wyf(init, edgesrc, edgetgt, n_interpolation_points, self.intervals_per_integer, self.min_num_intervals,
                                  self.alpha, self.beta, self.gamma, self.max_iter, self.combine, self.randseed)
        ...
        if (self.algo == 'Exact_wyf'):
            posres, t = Exact_wyf(init, edgesrc, edgetgt, self.alpha,
                              self.beta, self.gamma, self.max_iter, self.randseed, self.filename)
        if (self.algo == 'BH'):
