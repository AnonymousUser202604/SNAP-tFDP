...
public class LinLogLayout {
    ...
    /**
     * Returns, for each node in a given list, an initial position read from a file.
     * The file format is:
     * line 1: x y [z]
     * line 2: x y [z]
     * ...
     * where line i (1-based) contains the coordinates for "node (i-1)".
     *
     * Important: Since the node list can have an arbitrary iteration order,
     * "node 0/1/2/..." is defined here as the nodes sorted by {@code node.name}
     * (ascending lexicographic order). This makes the mapping deterministic.
     *
     * @param nodes node list.
     * @param is3d if true, z is taken from the 3rd token if present (else 0.0).
     * @param filename name of the file to read from.
     * @return map from each node to its initial position.
     */
    private static Map<Node,double[]> makeInitialPositionsFromFile(
            List<Node> nodes, boolean is3d, String filename) {
        List<Node> orderedNodes = new ArrayList<Node>(nodes);
        ...
        try {
            ...
            while ((line = file.readLine()) != null) {
                ...
                if (nodeIndex >= orderedNodes.size()) {
                    System.err.println("Too many lines in initial position file: " + filename);
                    System.err.println("Expected " + orderedNodes.size() + " node lines.");
                    System.exit(1);
                }

                StringTokenizer st = new StringTokenizer(line);
                if (st.countTokens() < 2) {
                    System.err.println("Invalid initial position file format at line " + lineNumber + ": " + filename);
                    System.err.println("Expected: <x> <y> [<z>]");
                    System.exit(1);
                }
                ...
                try {
                    ...
                    z = 0.0;
                    if (is3d && st.hasMoreTokens()) {
                        z = Double.parseDouble(st.nextToken());
                    }
                } catch (NumberFormatException e) {
                    System.err.println("Invalid number in initial position file at line " + lineNumber + ": " + filename);
                    System.err.println(e);
                    System.exit(1);
                    return null; // unreachable
                }

                Node node = orderedNodes.get(nodeIndex);
            ...
            if (nodeIndex != orderedNodes.size()) {
                System.err.println("Not enough lines in initial position file: " + filename);
                System.err.println("Expected " + orderedNodes.size() + " node lines, got " + nodeIndex + ".");
                System.exit(1);
            }
        } catch (IOException e) {
            System.err.println("Exception while reading the initial positions:");
            System.err.println(e);
            System.exit(1);
        } finally {
            if (file != null) {
                try { file.close(); } catch (IOException e) { /* ignore */ }
            }
        }
        return result;
    }
	 ...
	 * The first line is the algorithm running time in seconds (6 decimal places).
	 *
	 * @param nodeToPosition map from each node to its layout position.
	 * @param nodeToCluster map from each node to its cluster.
	 * @param filename name of the file to write into.
	 * @param runtimeSec algorithm running time in seconds, written as the first line.
	 */
	private static void writePositions(Map<Node,double[]> nodeToPosition, 
            Map<Node,Integer>nodeToCluster, String filename, double runtimeSec) {
		try {
	...
	public static void main(final String[] args) {
		if ((args.length != 3 && args.length != 4) || (!args[0].equals("2") && !args[0].equals("3")) ) {
			System.out.println(
				  "Usage: java LinLogLayout <dim> <inputfile> <outputfile> [<initposfile>]\n"
                ...
                + "<dim> must be 2 or 3.\n\n"
                + "Optional init position file format:\n"
                + "Line i (starting with 1) contains the coordinates for node (i-1):\n"
                + "<x-coordinate> <y-coordinate> [<z-coordinate>]\n"
                + "Nodes are ordered by their name (lexicographic order).\n\n"
				+ "Input file format:\n"
        ...
        boolean is3d = args[0].equals("3");
		Map<Node,double[]> nodeToPosition = (args.length == 4)
                ? makeInitialPositionsFromFile(nodes, is3d, args[3])
                : makeInitialPositions(nodes, is3d);
		...
		double runtimeSec = (System.nanoTime() - startTime) / 1e9;
		writePositions(nodeToPosition, nodeToCluster, args[2], runtimeSec);
